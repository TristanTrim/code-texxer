...
def getProgramIO(program):
    
    # the string that we will return filled with tasty program output and user input #
    string = ""

    # the subprocess running the program #
    print("getProgramIO.py running: "+program)
    child = subprocess.Popen(["java",program],bufsize = 0, universal_newlines = True, stdout = subprocess.PIPE, stdin = subprocess.PIPE )
